...
def estimation_bell(y, t, marginal, t0_min, t0_max, iter=50):

    n = 30
    if marginal in ["Poisson", "ZIP"]:
        d = 6
        bounds = [np.array([np.log((np.min(y + 1))) , -np.inf, -np.inf, t.min() - 0., -np.inf, -np.inf]),
                  np.array([np.log((np.max(y))) , np.inf, np.inf, t.max() + 0., np.inf, np.inf])]
    ...
    b[:, 2] += 5
    #print(b[2, 1])
    b[:, 3] = np.random.uniform(t0_min, t0_max, n)
    if d == 7:
        b[:, -3] += 1
...
def estimation_valley(y, t, marginal, t0_min, t0_max, iter=50):

    n = 30
    if marginal in ["Poisson", "ZIP"]:
        d = 7
        bounds = [np.array([np.log((np.min(y + 1))) , -np.inf, -np.inf, t0_min - 0.5, -np.inf, -np.inf, 0]),
                  np.array([np.log((np.max(y))) , np.inf, np.inf, t0_max + 0.5, np.inf, np.inf, np.inf])]
    elif marginal in ["NB", "ZINB"]:
        d = 8
        bounds = [np.array([np.log(np.min(y) + 1) , -np.inf, -np.inf, t0_min - 0.5, 1, 0, 0, 0]),
                  np.array([np.log(np.max(y) + 1) , np.inf, np.inf, t0_max + 0.5, 100, 100, 100, np.inf])]
    else:
        raise ValueError("Enter a valid marginal distribution: [NB, ZINB, Poisson, ZIP]!")
